...
def create_opening_node(pgn)
    ...
    parents, tagging, tag, rootNode = [], false, nil, nil

    pgn.split(' ').each_with_index do |move, i|
        case move
        when '(' then parents.push(parents.last)
        ...
        when '}' then tagging = false
            ...
            if tagging && !tag
                ...
                rootNode = node if i == 0
                parent = parents.pop
                parent.add_child_node(node) if parent
                parents.push(node)
                tag = nil
            end
        end
